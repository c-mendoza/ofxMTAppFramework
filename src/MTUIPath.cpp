...
void MTUIPath::setup(std::shared_ptr<ofPath> p, MTView* view)
{
   setup(p, view, (unsigned int) pathOptionFlags.to_ulong());
}

void MTUIPath::setup(std::shared_ptr<ofPath> p, MTView* view, unsigned int options)
{
   path = p;
   pathHandles.clear();
...
void MTUIPath::draw()
{
   if (isVisible)
   {
      ...
      for (int i = 0; i < pathHandles.size(); i++)
      {
         ...
         {
            ofDrawLine(handle->getCP2Handle()->getFrameCenter(), handle->getPointHandle()->getFrameCenter());

            ofDrawLine(handle->getCP1Handle()->getFrameCenter(), previous->getPointHandle()->getFrameCenter());
         }
...
void MTUIPath::handlePressed(MTUIPathVertexHandle* handle, ofMouseEventArgs& args)
{
   if (args.button == 0)
   {
      auto it = std::find_if(pathHandles.begin(),
                             pathHandles.end(),
                             [&](std::shared_ptr<MTUIPathVertexHandle> const& current) { return current.get() == handle; });

      // Transform lineTo <-> bezierTo
#pragma mark VERTEX TRANSFORM
      if (ofGetKeyPressed(OF_KEY_SHIFT) && pathOptionFlags.test(CanConvertPoints))
      {
...
void MTUIPathVertexHandle::setup(std::weak_ptr<MTUIPath> uiPath, ofPath::Command com)
{
   ...
   toHandle->setFrameFromCenter(command.to, glm::vec2(MTUIPath::vertexHandleSize, MTUIPath::vertexHandleSize));

   cp1Handle = std::make_shared<MTUIHandle>("CP 1");
   cp1Handle->resizePolicy = MTViewResizePolicy::ResizePolicyNone;
   cp1Handle->setFrameFromCenter(command.cp1, glm::vec2(MTUIPath::vertexHandleSize, MTUIPath::vertexHandleSize));
   ...
   cp2Handle->setFrameFromCenter(command.cp2, glm::vec2(MTUIPath::vertexHandleSize, MTUIPath::vertexHandleSize));
   ...
   addEventListener(
       cp1Handle->mouseDraggedEvent.newListener([this](ofMouseEventArgs& args) { updateCommand(); }, OF_EVENT_ORDER_AFTER_APP));

   addEventListener(
       cp2Handle->mouseDraggedEvent.newListener([this](ofMouseEventArgs& args) { updateCommand(); }, OF_EVENT_ORDER_AFTER_APP));
...
{
   cp1Handle->setFrameFromCenter(command.cp1, glm::vec2(MTUIPath::cpHandleSize, MTUIPath::cpHandleSize));
   ...
   cp2Handle->setFrameFromCenter(command.cp2, glm::vec2(MTUIPath::cpHandleSize, MTUIPath::cpHandleSize));
...
void MTUIPathVertexHandle::updateCommand()
{
   ...
   {
      auto pos = toHandle->getFrameCenter();
      toHandle->setFrameCenter(glm::vec2(ofClamp(pos.x, uiPathPtr->region.getMinX(), uiPathPtr->region.getMaxX()),
                                         ofClamp(pos.y, uiPathPtr->region.getMinY(), uiPathPtr->region.getMaxY())));
   }
...
{
   wantsFocus = true;
   addEventListener(addedToSuperviewEvent.newListener([this](ofEventArgs& args) { scaleToScreen(); }));
}
...
void MTUIHandle::draw()
{
   ofSetColor(255);
   ofFill();
   ofDrawRectangle(0, 0, getFrameSize().x, getFrameSize().y);
...
void MTUIHandle::mouseDragged(int x, int y, int button)
{
   if (button == 0)
   {
      setFrameOrigin(getFrameOrigin() + (getContentMouse() - getContentMouseDown()));
   }
}
