...
void MTZoomableDocumentView::mouseScrolled(int x, int y, float scrollX, float scrollY)
{
	...
	auto startContentMouse = glm::clamp(getContentMouse(),
										glm::vec2(0, 0),
										glm::vec2(documentRect.getWidth(), documentRect.getHeight()));
...
//    ofLogVerbose("Scrolling") << "contentMouse: " << startContentMouse.x << " " << startContentMouse.y;
	...
	newContentMouse = glm::clamp(newContentMouse,
								 glm::vec2(0, 0),
								 glm::vec2(documentRect.getWidth(), documentRect.getHeight()));
...
void MTZoomableDocumentView::setupDocumentRect(float width,
											   float height,
