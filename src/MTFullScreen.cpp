...
void MTFullScreen::updateFullscreenDisplays()
{
   ...
   for (auto fsDisplay : displayOutputs)
   {
      ...
      fsDisplay->outputArea->setSize(oneWidth, outputHeight);
...
void priv::enterFullScreen()
{
   ...
   for (auto fsDisplay : displayOutputs)
   {
      ...
      auto fv = MTView::CreateView<MTFullScreenView>(name, fsDisplay, outputTexture);


      auto fsView = MTView::CreateView<MTFullScreenView>(name, fsDisplay, outputTexture);
...
void MTFullScreen::addFullScreenDisplay(std::shared_ptr<MTFullScreenDisplayInfo> fsDisplay)
{
   if (fsDisplay->outputArea == nullptr)
   {
      ...
      fsDisplay->outputArea = std::make_shared<ofRectangle>(xStart,
                                                            0,
    ...
    : MTView(name)
{
   ...
   outputMesh = outputTexture.getMeshForSubsection(0,
                                                   0,
                                                   0,
                                                   fullScreenDisplay->outputArea->getWidth(),
                                                   fullScreenDisplay->outputArea->getHeight(),
                                                   fullScreenDisplay->outputArea->x,
                                                   fullScreenDisplay->outputArea->y,
                                                   fullScreenDisplay->outputArea->getWidth(),
                                                   fullScreenDisplay->outputArea->getHeight(),
                                                   true,
                                                   OF_RECTMODE_CORNER);
...
void MTFullScreenView::updatePerspectiveTransform()
{
   enqueueUpdateOperation(
       [this]() {
          perspectiveMatrix = MTHomographyHelper::calculateHomography(getFrame(), perspectiveQuad->getOutline()[0].getVertices());
       });
}
