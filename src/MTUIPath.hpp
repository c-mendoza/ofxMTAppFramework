...
class MTUIPath : public std::enable_shared_from_this<MTUIPath>
{
 ...
 public:
	...
	* header file.
	*/
   void setup(std::shared_ptr<ofPath> path, MTView* view, unsigned int options);
   ...
   void setup(std::shared_ptr<ofPath> path, MTView* view);
   void draw();
...
class MTUIPathVertexHandle : public MTEventListenerStore
{
 ...
 public:
   ...
   std::shared_ptr<ofPath> getPath()
   {
      return uiPath.lock()->getPath();
   }  // Probably bad form, but meh
