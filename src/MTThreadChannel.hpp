//
// Created by Cristobal Mendoza on 10/14/19.
//

#ifndef NERVOUSSTRUCTUREOF_MTTHREADCHANNEL_HPP
#define NERVOUSSTRUCTUREOF_MTTHREADCHANNEL_HPP
...
///
/// MTThreadChannel makes it easy to safely and efficiently share data between
/// threads without the need for shared memory, mutexes, or other
/// synchronization techniques. Additionally, MTThreadChannel employs a
/// signalling system that allows receiving threads to sleep until new data
/// arrives or the MTThreadChannel is closed.
///
/// A single MTThreadChannel class is designed for one-way communication. In
/// most cases an additional MTThreadChannel can be used for two-way
/// communication.
///
/// Data is sent through the MTThreadChannel in a FIFO (first in, first out)
/// order, guaranteeing that that data will be received in the same order that
/// it was sent.
///
/// If multiple threads attempt to send data using the same MTThreadChannel, the
...
/// \tparam T The data type sent by the MTThreadChannel.
template<typename T>
class MTThreadChannel
{
 public:
   /// \brief Create a default MTThreadChannel.
   ///
   /// MTThreadChannel must be instantiated with a template parameter such as:
   /// ~~~~{.cpp}
   /// 	MTThreadChannel<ofPixels> myThreadChannel;
   /// ~~~~
   MTThreadChannel() : closed(false)
   {
   }
   ...
   ///
   /// MTThreadChannel::receive will not make a copy or reallocate data.
   ///
   /// ~~~~{.cpp}
   /// MTThreadChannel<ofPixels> myThreadChannel;
   ///
   /// // MTThreadChannel<ofPixels> declared elsewhere.
   ///
   ...
   /// \returns True if a new value was received or false if the MTThreadChannel was closed.
   ...
   ///
   /// MTThreadChannel::tryReceive is similar to MTThreadChannel::receive,
   /// except that it will not block the receiving thread.  If no data is
   /// available, it will return false and continue immediately.
   ///
   /// MTThreadChannel::tryReceive will not make a copy or reallocate data.
   ///
   /// Like MTThreadChannel::receive, in order to receive data, the user must
   ...
   /// MTThreadChannel<ofPixels> myThreadChannel;
   ///
   /// // MTThreadChannel<ofPixels> initialized elsewhere.
   ///
   /// ofPixels myPixelsToSet;
   ///
   ...
   /// \brief Close the MTThreadChannel.
   ///
   /// Closing the MTThreadChannel means that no new messages can be sent or
   /// received. All threads waiting to receive new values will be notified and
   /// all MTThreadChannel::receive and MTThreadChannel::tryReceive methods
   /// will return false.
   void close()
   {
      std::unique_lock<std::mutex> lock(mutex);
...
#endif  //NERVOUSSTRUCTUREOF_MTTHREADCHANNEL_HPP
